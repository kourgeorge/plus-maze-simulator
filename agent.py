...
class Agent:
    def __init__(self, motivation=config.RewardType.WATER, motivated_reward_value=1, non_motivated_reward_value=0):
        self._brain = BrainDQN(observation_size=PlusMaze.state_shape(),
                               num_actions=PlusMaze.num_actions(),
                               reward_discount=0,
                               learning_rate=config.BASE_LEARNING_RATE)

        self._memory_size = 1024
        self._memory = deque(maxlen=self._memory_size)
