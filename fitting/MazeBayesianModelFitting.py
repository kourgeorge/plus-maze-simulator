...
class MazeBayesianModelFitting:
	...
	def __init__(self, env, experiment_data, model, parameters_space, method, n_calls):
		self.env = env
	...
	def _calc_experiment_likelihood(self, parameters):
		...
		aic, bic, likelihood_stage, meanL, meanNLL = fitting_utils.analyze_fitting(rat_data_with_likelihood, 'likelihood', len(parameters))
	...
	def optimize(self):
		if self.method.name in [fitting_config.OptimizationMethod.BAYESIAN.name, fitting_config.OptimizationMethod.HYBRID.name]:
			search_result = gp_minimize(self._calc_experiment_likelihood, self.parameters_space, n_calls=self.n_calls)
			x0 = search_result.x
			parameters = x0

		if self.method.name in [fitting_config.OptimizationMethod.NEWTON.name, fitting_config.OptimizationMethod.HYBRID.name]:
			bounds = [bound.bounds for bound in self.parameters_space]
			x0 = np.array([param.rvs()[0] for param in self.parameters_space]) if fitting_config.OPTIMIZATION_METHOD == fitting_config.OptimizationMethod.NEWTON else x0
			print(f"\t\t-----Finished Bayesian parameter estimation: {x0} -----")
			has_attn_importance_param = any(param.name == 'initial_attn_importance' for param in self.parameters_space)
			if self.model[2] == ACFTable and has_attn_importance_param:
				def constraint(params):
					resolved_params = fitting_utils.resolve_parameters(params, *self.model)
					return 1-int(utils.is_valid_attention_weights(resolved_params['initial_attn_importance']))

				constraints = {'type': 'eq', 'fun': constraint}  # Inequality constraint: x + y <= 1
	...
	@staticmethod
	def all_subjects_all_models_optimization(env, animals_data_folder, all_models, method=fitting_config.OPTIMIZATION_METHOD, n_calls=fitting_config.FITTING_ITERATIONS):
		...
		for subject_id, (file_name,curr_rat) in enumerate(animal_data):
			...
			for curr_model in all_models:
				...
				search_result, experiment_stats, rat_data_with_likelihood = \
					MazeBayesianModelFitting(env, curr_rat, model, parameters_space, method, n_calls).optimize()
				...
				rat_data_with_likelihood["algorithm"] = "{}_{}".format(fitting_config.OPTIMIZATION_METHOD, n_calls)

				results_df = pd.concat([results_df, rat_data_with_likelihood], ignore_index=True)
			results_df.to_csv(f'fitting/Results/Rats-Results/fitting_results_dimensional{timestamp}_{n_calls}_tmp.csv')
		results_df.to_csv(f'fitting/Results/Rats-Results/asymmetric_fitting_results_dimensional{timestamp}_{n_calls}.csv')
...
if __name__ == '__main__':
	...
	MazeBayesianModelFitting.all_subjects_all_models_optimization(
	    PlusMazeOneHotCues2ActiveDoors(stimuli_encoding=10), fitting_config.MAZE_ANIMAL_DATA_PATH,
