...
def PlusMazeExperimentFitting(env: PlusMaze, agent: MotivatedAgent, experiment_data, dashboard=False):
    ...
    fitting_info['model_variables'] = fitting_info['model_variables'].astype(object)
    fitting_info['stimuli_value'] = np.nan
    fitting_info['action_bias'] = np.nan
    fitting_info['optimization_data'] = np.nan
    fitting_info['optimization_data'] = fitting_info['optimization_data'].astype(object)
    env.reset_trial()
    ...
    while trial < len(fitting_info):
        ...
        if completed_trial(fitting_info, trial):
            ...
            fitting_info.at[trial, 'likelihood'] = np.round(likelihood,10)
            ...
            chosen_action = np.argmax(agent.get_memory().last(1)[0][1])
            fitting_info.at[trial, 'stimuli_value'] = agent.get_brain().get_model().get_observations_values(curr_state, agent.get_motivation())[0][chosen_action]
            fitting_info.at[trial, 'action_bias'] = agent.get_brain().get_model().get_bias_values(agent.get_motivation())[chosen_action]
